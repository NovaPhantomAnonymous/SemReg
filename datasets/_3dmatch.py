...
from datasets.indoor_2d3d_fast import IndoorDataset2D3D
...
class _3DMatch(Dataset):
    ...
    def __init__(self, config, split, data_augmentation=True):
        ...
        self.indoor2d3d = IndoorDataset2D3D(
            infos=self.infos, 
            base_dir=self.base_dir, 
            rgbd_dir=config.rgbd_root,
            data_augmentation=data_augmentation, 
            use_precompute_2dfeat=config.use_precomputed_2dfeat
        )
    ...
    def __getitem__(self, item, debug=False):
        ...
        if self.data_augmentation:
            ...
            aug_src = np.random.rand(1)[0] > 0.5
        ...
        # src_2d3d, tgt_2d3d, mutual_mask_labels, all_mask_labels = self.indoor2d3d._get_image_pair(
        #     item, src_pcd, tgt_pcd, 
        #     aug_src=aug_src if self.data_augmentation else None, 
        #     rot_ab=rot_ab if self.data_augmentation else None
        # )
        
        src_2d3d, tgt_2d3d = self.indoor2d3d._get_image_pair(
            item, src_pcd, tgt_pcd, 
            aug_src=aug_src if self.data_augmentation else None, 
            rot_ab=rot_ab if self.data_augmentation else None
        )
        ...
        tsfm = tsfm.astype(np.float32)

        # return src_pcd, tgt_pcd, src_feats, tgt_feats, \
        #     correspondences, rot, trans, gt_cov, src_2d3d, tgt_2d3d, \
        #     mutual_mask_labels, all_mask_labels, tsfm
        return src_pcd, tgt_pcd, src_feats, tgt_feats, \
            correspondences, rot, trans, gt_cov, src_2d3d, tgt_2d3d, tsfm
